...
export class InvoiceService {
   ...
   */
  async generateInvoice(userId: number, orderId: number, generateInvoiceDto: GenerateInvoiceDto): Promise<InvoiceResponseDto | Buffer> {
    try {
      this.logger.log(`🧾 Generating invoice for order ${orderId} by user ${userId}`);
      ...
      // Format invoice data
      const invoiceData = await this.formatInvoiceData(order, payment, latestTracking, generateInvoiceDto.notes);
      ...
      try {
        ...
        this.logger.error('PDF generation failed, falling back to JSON:', pdfError);
        return invoiceData;
      }
   ...
   */
  private async formatInvoiceData(order: Order, payment: Payment | null, tracking: OrderTracking | null, notes?: string): Promise<InvoiceResponseDto> {
    ...
    const customer = {
      ...
      address: {
        street: order.delivery_address.address1,
    ...
    // Format order items
    const items = await Promise.all(order.order_items.map(async (orderItem) => {
      const customizations = await this.getCustomizationDetails(orderItem.customizations);
      
      return {
        name: orderItem.item.name,
        description: orderItem.item.short_desc || orderItem.item.long_desc || '',
   ...
   */
  private async getCustomizationDetails(customizations: any): Promise<string[]> {
    if (!customizations) {
    ...
    try {
      const customizationsData = typeof customizations === 'string' 
      ...
      if (Array.isArray(customizationsData)) {
        ...
        if (allSelectedOptions.length === 0) {
          this.logger.debug('No selected_options found in customization groups');
          return [];
        }
        ...
        this.logger.debug(`Found ${customizationItems.length} customization items`);
        
        customizationItems.forEach(item => {
          // Get the first price entry (usually INR) or default to 0
          const price = item.prices?.find(p => p.currency === 'INR')?.base_price || 
                       item.prices?.[0]?.base_price || 0;
      ...
      if (!customizationsData.selected_options || !Array.isArray(customizationsData.selected_options)) {
      ...
      const customizationItems = await this.itemRepository
        .createQueryBuilder('item')
        .leftJoinAndSelect('item.prices', 'price')
        .where('item.id IN (:...ids)', { ids: customizationsData.selected_options })
        .getMany();
  ...
  private async generatePDF(invoiceData: InvoiceResponseDto): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        doc.text(`Date: ${new Date(invoiceData.invoice_date).toLocaleDateString('en-IN')}`, 100, 58);
        ...
        invoiceData.items.forEach((item, index) => {
          ...
          if (item.customizations && item.customizations.length > 0) {
            this.logger.debug(`Adding ${item.customizations.length} customizations for item: ${item.name}`);
            doc.fontSize(8)
               .font('Helvetica')
               .fillColor(darkGray);
        ...
        doc.fillColor('black')
           ...
           .text(`Rs. ${invoiceData.pricing.subtotal.toFixed(2)}`, 500, yPosition);

        yPosition += 15;
        doc.text(`Delivery Fee:`, 360, yPosition)
           .text(`Rs. ${invoiceData.pricing.delivery_fee.toFixed(2)}`, 500, yPosition);

        yPosition += 15;
        doc.text(`Tax:`, 360, yPosition)
           .text(`Rs. ${invoiceData.pricing.tax_amount.toFixed(2)}`, 500, yPosition);
        ...
        doc.fontSize(14)
           ...
           .text(`Rs. ${invoiceData.pricing.total_amount.toFixed(2)}`, 500, yPosition);
        ...
        if (invoiceData.payment) {
          ...
          doc.fillColor('black')
             .fontSize(10)
             .font('Helvetica')
             .text(`Method: ${invoiceData.payment.method.toUpperCase()}`, 50, yPosition);
          yPosition += 15;
          doc.text(`Status: ${invoiceData.payment.status.toUpperCase()}`, 50, yPosition);
          
          // Only show transaction ID if not COD and transaction_id exists
          if (invoiceData.payment.method.toLowerCase() !== 'cod' && invoiceData.payment.transaction_id) {
            yPosition += 15;
            doc.text(`Transaction ID: ${invoiceData.payment.transaction_id}`, 50, yPosition);
          }
        }

        // Delivery Information (only show if agent details are available)
        if (invoiceData.delivery && invoiceData.delivery.agent_name && invoiceData.delivery.agent_phone) {
          yPosition += 30;
          doc.fillColor(secondaryColor)
             .fontSize(12)
             .font('Helvetica-Bold')
        ...
        doc.text('For support, contact us at support@example.com', 50, 780, { align: 'center' });
   ...
   */
  async getInvoiceData(userId: number, orderId: number): Promise<InvoiceResponseDto> {
    const invoiceData = await this.generateInvoice(userId, orderId, { format: InvoiceFormat.JSON });
