...
export class FavoriteItemDto {
  ...
  @ApiProperty({ description: 'Item description', example: 'Authentic Hyderabadi style biryani' })
  description: string;

  @ApiProperty({ description: 'Item images', type: [String], example: ['https://example.com/biryani.jpg'] })
  images: string[];

  @ApiProperty({ description: 'Item price', example: 299.00 })
  price: number;
  ...
  @ApiProperty({ description: 'When item was favorited', example: '2025-01-15T10:30:00Z' })
  favorited_at: Date;
...
export class FavoriteRestaurantDto {
  ...
  @ApiProperty({ description: 'Restaurant description', example: 'Best South Indian cuisine' })
  description: string;

  @ApiProperty({ description: 'Restaurant logo URL', example: 'https://example.com/logo.jpg' })
  logo_url: string;
  ...
  @ApiProperty({ description: 'When restaurant was favorited', example: '2025-01-15T10:30:00Z' })
  favorited_at: Date;

  @ApiProperty({ description: 'Is restaurant favorited by user', example: true })
  is_favorite: boolean;
}
...
export class AllFavoritesResponseDto {
  ...
  @ApiProperty({ description: 'Response message', example: 'Favorites retrieved successfully' })
  message: string;
  ...
  @ApiProperty({
    ...
    type: 'object',
    properties: {
      restaurants: { type: 'array', items: { $ref: '#/components/schemas/FavoriteRestaurantDto' } },
      items: { type: 'array', items: { $ref: '#/components/schemas/FavoriteItemDto' } },
      summary: {
        type: 'object',
        properties: {
...
export class FavoriteStatusDto {
  ...
  @ApiProperty({ description: 'When favorited (null if not favorited)', example: '2025-01-15T10:30:00Z', nullable: true })
...
export class ToggleFavoriteResponseDto {
  ...
  @ApiProperty({ description: 'Action performed', example: 'added', enum: ['added', 'removed'] })
  action: 'added' | 'removed';

  @ApiProperty({ description: 'Response message', example: 'Item added to favorites' })
  message: string;

  @ApiProperty({ description: 'Current favorite status', type: FavoriteStatusDto })
