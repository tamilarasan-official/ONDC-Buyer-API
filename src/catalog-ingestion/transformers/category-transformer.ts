...
export class CategoryTransformer extends BaseTransformer {
   ...
   */
  transform(categoryData: ONDCCategory, store: Store, existingCategory?: Category): Category {
    ...
    try {
      ...
      if (categoryData.parent_category_id) {
        category.parent_category_id = this.parseInteger(categoryData.parent_category_id);
      }
      ...
      this.logger.log(`Transformed category: ${category.reference_id} - ${category.name} (${category.type})`);
  ...
  validateCategory(category: Category): { isValid: boolean; errors: string[] } {
    ...
    if (!['custom_menu', 'custom_group'].includes(category.type)) {
      errors.push('Category type must be either "custom_menu" or "custom_group"');
    }
    
    if (category.parent_category_id !== undefined && category.parent_category_id <= 0) {
