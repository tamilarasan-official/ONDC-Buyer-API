...
export class OfferTransformer extends BaseTransformer {
  ...
  transform(
    ...
    try {
      ...
      offer.banner_image_url = this.sanitizeUrl(
        offerData.descriptor.images?.[0],
        "",
      );
      ...
      this.logger.log(
        `Transformed offer: ${offer.reference_id} - ${offer.offer_code}`,
      );
  ...
  private parseOfferTiming(offerData: ONDCOffer, offer: Offers): void {
    if (offerData.time?.range) {
      offer.valid_from =
        this.parseDateTime(offerData.time.range.start, new Date()) ||
        new Date();
      offer.valid_to =
        this.parseDateTime(
          offerData.time.range.end,
    ...
    if (offer.valid_to <= offer.valid_from) {
      ...
      offer.valid_to = new Date(
        offer.valid_from.getTime() + 24 * 60 * 60 * 1000,
      ); // Add 1 day
    }
  }
  ...
  transformBenefits(offerData: ONDCOffer): Array<{
    ...
    if (benefitData.value_type) {
      const benefit = {
        ...
        benefit_cap: benefitData.value_cap
          ? this.sanitizeString(benefitData.value_cap, 255)
          : undefined,
        benefit_item_count: benefitData.item_count
          ? this.parseInteger(benefitData.item_count)
          : undefined,
      };
  ...
  validateOffer(offer: Offers): { isValid: boolean; errors: string[] } {
    ...
    if (
      offer.valid_from &&
      offer.valid_to &&
      offer.valid_to <= offer.valid_from
    ) {
