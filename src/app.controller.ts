  ...
  async ondcWebhook(@Body() catalogData: any) {
    try {
      ...
      if (!catalogData.message.catalog) {
        ...
        this.logger.log(
          `📋 Message fields: ${JSON.stringify(Object.keys(catalogData.message))}`,
        );
      ...
      this.logger.log(
        `📊 Valid catalog response with ${providersCount} provider(s)`,
      );
      ...
      this.logger.log(
        `🔄 Processing catalog with ${providersCount} provider(s)`,
      );
      ...
      this.logger.error(
        `❌ ONDC webhook failed: ${error.message}`,
        error.stack,
      );
      this.logger.error(
        `📋 Problematic payload: ${JSON.stringify(catalogData)}`,
      );
  ...
  private logDetailedItemInfo(catalogData: any) {
    try {
      ...
      providers.forEach((provider: any, providerIndex: number) => {
        this.logger.log(
          `🏪 Provider ${providerIndex + 1}: ${provider.descriptor?.name || "Unknown"}`,
        );
        this.logger.log(`   ID: ${provider.id}`);
        this.logger.log(`   Items Count: ${provider.items?.length || 0}`);
        this.logger.log(
          `   Categories Count: ${provider.categories?.length || 0}`,
        );
        ...
        if (provider.categories && provider.categories.length > 0) {
          ...
          provider.categories.forEach((category: any, catIndex: number) => {
            this.logger.log(
              `      ${catIndex + 1}. ${category.descriptor?.name || "Unknown"} (ID: ${category.id})`,
            );
            this.logger.log(
              `         Type: ${category.tags?.find((tag: any) => tag.code === "type")?.list?.[0]?.value || "regular"}`,
            );
          });
        }
        ...
        if (provider.items && provider.items.length > 0) {
          ...
          provider.items.forEach((item: any, itemIndex: number) => {
            this.logger.log(
              `      ${itemIndex + 1}. ${item.descriptor?.name || "Unknown"} (ID: ${item.id})`,
            );
            this.logger.log(
              `         Type: ${item.tags?.find((tag: any) => tag.code === "type")?.list?.[0]?.value || "item"}`,
            );
            this.logger.log(
              `         Parent Item ID: ${item.parent_item_id || "None"}`,
            );
            this.logger.log(
              `         Price: ${item.price?.value || "N/A"} ${item.price?.currency || ""}`,
            );
            ...
            if (item.tags && item.tags.length > 0) {
              ...
              item.tags.forEach((tag: any) => {
                this.logger.log(
                  `            ${tag.code}: ${JSON.stringify(tag.list)}`,
                );
              });
            }
            ...
            if (item.parent_item_id) {
              this.logger.log(
                `         🔗 This is a customization/variant item linked to parent: ${item.parent_item_id}`,
              );
            }
          });

          // 🔍 CUSTOMIZATION ANALYSIS: Find items with parent_item_id relationships
          const customizationItems = provider.items.filter(
            (item: any) => item.parent_item_id,
          );
          const mainItems = provider.items.filter(
            (item: any) => !item.parent_item_id,
          );
          ...
          this.logger.log(
            `      Customization/Variant Items: ${customizationItems.length}`,
          );
          ...
          if (customizationItems.length > 0) {
            ...
            customizationItems.forEach((item: any) => {
              this.logger.log(
                `         ${item.descriptor?.name} -> Parent: ${item.parent_item_id}`,
              );
            });
          }
        }
      ...
      this.logger.error(
        `❌ Error logging detailed item info: ${error.message}`,
      );
    }
  }
}
