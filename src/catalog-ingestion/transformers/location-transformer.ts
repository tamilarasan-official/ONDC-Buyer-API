...
export class LocationTransformer extends BaseTransformer {
  ...
  transform(
    ...
    store: Store,
    existingLocation?: StoreLocation,
  ): StoreLocation {
    ...
    try {
      ...
      if (gpsCoords) {
        ...
        this.logWarning(
          `Invalid GPS coordinates for location ${locationData.id}: ${locationData.gps}`,
        );
      ...
      if (locationData.address) {
        location.address_locality = this.sanitizeString(
          locationData.address.locality,
          255,
        );
        location.address_street = this.sanitizeString(
          locationData.address.street,
          255,
        );
        location.address_city = this.sanitizeString(
          locationData.address.city,
          100,
        );
        location.address_area_code = this.sanitizeString(
          locationData.address.area_code,
          10,
        );
      ...
      if (locationData.circle?.radius) {
        location.delivery_radius_km = this.parseFloat(
          locationData.circle.radius.value,
        );
        location.delivery_radius_unit = this.sanitizeString(
          locationData.circle.radius.unit,
          10,
          "km",
        );
      }
      ...
      this.logger.log(
        `Transformed location: ${location.reference_id} at ${location.gps_lat},${location.gps_lng}`,
      );
  ...
  private parseLocationTiming(
    ...
    if (locationData.time.schedule?.holidays) {
      location.schedule_holidays = this.sanitizeHolidays(
        locationData.time.schedule.holidays,
      );
    }
  }
   ...
   */
  validateLocation(location: StoreLocation): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
