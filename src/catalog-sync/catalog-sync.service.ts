...
export class CatalogSyncService {
  ...
  constructor(
    ...
    this.enabledCronJobs = this.configService
      .get<string>("ENABLED_CRON_JOBS", "full_sync,incremental_sync")
      .split(",")
      .map((job) => job.trim());
  ...
  async handleFullCatalogSync(): Promise<void> {
    ...
    try {
      ...
      const totalStats = this.initializeStats();
      ...
      for (const city of cities) {
        ...
        try {
          ...
          const searchResult =
            await this.ondcSearchService.performCatalogRefresh(city);

          if (searchResult.success) {
            this.logger.log(
              `🔍 Search request sent successfully for ${city}. Message ID: ${searchResult.message_id}`,
            );
            this.logger.log(
              `📞 Waiting for multiple ON_SEARCH webhook calls for ${city}`,
            );
            totalStats.cities_processed++;
          } else {
            this.logger.warn(
              `⚠️ Search request failed for city: ${city}. Status: ${searchResult.ack_status}`,
            );
            totalStats.city_errors.push(
              `${city}: Search failed - ${searchResult.ack_status}`,
            );
          }
          ...
          this.logger.error(
            `❌ Failed to process city ${city}: ${cityError.message}`,
            cityError.stack,
          );
      ...
      const duration = Date.now() - startTime;
      this.logger.error(
        `💥 FULL catalog sync failed after ${duration}ms: ${error.message}`,
        error.stack,
      );
  ...
  async handleIncrementalCatalogSync(): Promise<void> {
    ...
    if (currentHour < 8 || currentHour > 22) {
      this.logger.log(
        `⏰ Skipping incremental sync - outside business hours (${currentHour}:00)`,
      );
      return;
    }
    ...
    try {
      ...
      for (const city of primaryCities) {
        ...
        try {
          ...
          if (searchResult.success) {
            this.logger.log(
              `🔍 Incremental search request sent for ${city}. Message ID: ${searchResult.message_id}`,
            );
            this.logger.log(
              `📞 Waiting for ON_SEARCH webhook calls for ${city}`,
            );
            totalStats.cities_processed++;
          } else {
            this.logger.warn(
              `⚠️ Incremental search failed for ${city}. Status: ${searchResult.ack_status}`,
            );
            totalStats.city_errors.push(
              `${city}: Search failed - ${searchResult.ack_status}`,
            );
          ...
          this.logger.error(
            `❌ Incremental sync failed for ${city}: ${cityError.message}`,
          );
          totalStats.city_errors.push(`${city}: ${cityError.message}`);
        }
      ...
      const duration = Date.now() - startTime;
      this.logger.error(
        `💥 INCREMENTAL catalog sync failed after ${duration}ms: ${error.message}`,
        error.stack,
      );
  ...
  async handleHealthCheckSync(): Promise<void> {
    ...
    try {
      ...
      if (searchResult.success) {
        this.logger.log(
          `✅ HEALTH CHECK passed - Search request sent successfully in ${duration}ms. Message ID: ${searchResult.message_id}`,
        );
      } else {
        this.logger.warn(
          `⚠️ HEALTH CHECK warning - Search request failed in ${duration}ms. Status: ${searchResult.ack_status}`,
        );
      }
    } catch (error) {
      const duration = Date.now() - startTime;
  ...
  async triggerFullSync(cities?: string[]): Promise<{
    ...
    try {
      ...
      for (const city of targetCities) {
        ...
        if (searchResult.success) {
          this.logger.log(
            `🔍 Manual search request sent for ${city}. Message ID: ${searchResult.message_id}`,
          );
          totalStats.cities_processed++;
        } else {
          this.logger.warn(
            `⚠️ Manual search failed for ${city}. Status: ${searchResult.ack_status}`,
          );
          totalStats.city_errors.push(
            `${city}: Search failed - ${searchResult.ack_status}`,
          );
      ...
      return {
        success: true,
        message:
          "Manual search requests sent successfully. Catalog data will be received via ON_SEARCH webhooks.",
        stats: totalStats,
        duration,
      };
    } catch (error) {
      const duration = Date.now() - startTime;
      this.logger.error(
        `💥 MANUAL full catalog sync failed: ${error.message}`,
        error.stack,
      );
  ...
  async triggerIncrementalSync(cities?: string[]): Promise<{
    ...
    try {
      ...
      for (const city of targetCities) {
        ...
        if (searchResult.success) {
          this.logger.log(
            `🔍 Manual incremental search sent for ${city}. Message ID: ${searchResult.message_id}`,
          );
          totalStats.cities_processed++;
        } else {
          this.logger.warn(
            `⚠️ Manual incremental search failed for ${city}. Status: ${searchResult.ack_status}`,
          );
          totalStats.city_errors.push(
            `${city}: Search failed - ${searchResult.ack_status}`,
          );
      ...
      const duration = Date.now() - startTime;
      this.logger.log(
        `🎉 MANUAL incremental catalog sync completed in ${duration}ms`,
      );

      return {
        success: true,
        message:
          "Manual incremental search requests sent successfully. Catalog data will be received via ON_SEARCH webhooks.",
        stats: totalStats,
        duration,
      };
    } catch (error) {
      const duration = Date.now() - startTime;
      this.logger.error(
        `💥 MANUAL incremental catalog sync failed: ${error.message}`,
        error.stack,
      );
  ...
  private getTargetCities(): string[] {
    ...
    return this.configService
      .get<string>("SYNC_TARGET_CITIES", defaultCities.join(","))
      .split(",")
      .map((city) => city.trim());
  }
  ...
  private getPrimaryCities(): string[] {
    ...
    return this.configService
      .get<string>("SYNC_PRIMARY_CITIES", primaryCities.join(","))
      .split(",")
      .map((city) => city.trim());
  }
