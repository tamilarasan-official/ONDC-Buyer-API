...
export class FavoritesController {
  ...
  @ApiOperation({
    ...
    description: 'Add or remove an item from user favorites. If item is already favorited, it will be removed. If not favorited, it will be added.',
  })
  @ApiParam({
    name: 'itemId',
  ...
  @ApiOperation({
    ...
    description: 'Retrieve all items favorited by the authenticated user, with optional location for distance calculation.',
  })
  @ApiQuery({
    name: 'lat',
    required: false,
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        message: { type: 'string', example: 'Favorite items retrieved successfully' },
        data: {
          type: 'array',
      ...
      properties: {
        ...
        data: {
          ...
          properties: {
            ...
            favorited_at: { type: 'string', example: '2025-01-15T10:30:00Z', nullable: true },
          },
        },
      },
    },
  ...
  @ApiOperation({
    ...
    description: 'Add or remove a restaurant from user favorites. If restaurant is already favorited, it will be removed. If not favorited, it will be added.',
  })
  @ApiParam({
    name: 'storeId',
  ...
  async toggleFavoriteRestaurant(@Req() req: any, @Param('storeId') storeId: number) {
    const userId = req.user?.id;
  ...
  @ApiOperation({
    ...
    description: 'Retrieve all restaurants favorited by the authenticated user, with optional location for distance calculation.',
  })
  @ApiQuery({
    name: 'lat',
    required: false,
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        success: { type: 'boolean', example: true },
        message: { type: 'string', example: 'Favorite restaurants retrieved successfully' },
        data: {
          type: 'array',
  ...
  async getFavoriteRestaurants(
    ...
    const userLat = lat ? parseFloat(lat) : undefined;
    const userLng = lng ? parseFloat(lng) : undefined;
    return this.favoritesService.getFavoriteRestaurants(userId, userLat, userLng);
  }
  ...
  async checkRestaurantFavorite(@Req() req: any, @Param('storeId') storeId: number) {
    const userId = req.user?.id;
  ...
  @ApiOperation({
    ...
    description: 'Retrieve all favorites (both items and restaurants) for the authenticated user in a single response, grouped by type.',
  })
  @ApiQuery({
    name: 'lat',
    required: false,
